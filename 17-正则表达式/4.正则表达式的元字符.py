...
[sunck]        匹配's' 'u''n'....中任意一个字符
[a-z]          匹配任意小写字母
[A-Z]          匹配任意大写字母
[0-9]          匹配任意数字，类似[0123456789]
[0-9a-zA-Z]    匹配任意的数字和字母
[0-9a-zA-Z_]   匹配任意的数字和字母和下划线
[^sunck]       匹配除了”sunck“这几个字符以外的所有字符，中括号里的^称为脱字符，表示不匹配集合中的字符
[^0-9]         匹配所有的非数字字符
\d             匹配所有的数字，效果同[0-9]
\D             匹配非数字字符，效果同[^0-9]
...
\s             匹配任意的非空白符，效果同[^ \f\n\r\t]
"""
print(re.search(".","sunck is 6 a good man"))
print(re.search("[0123456789]","sunck is 6 a good man"))
print(re.search("[sunck]","sunck is 6 a good man"))
...
\b     匹配一个单词的边界，也就是只单词和空格间的位置
"er\b"可以匹配nerver,不能匹配nerve
\B     匹配非单词边界
"er\B"可以匹配nerve,不能匹配never
"""
...
"""
说明：下方的x，y，z，均为假设的普通字符，n,m 均为非负整数，不是正则表达式的元字符
（xyz）   匹配小括号内的xyz（作为一个整体去匹配
x?        匹配0个或者1个x
x*        匹配0个或者任意多个x   #贪婪匹配（尽可能多的匹配）（.* 表示匹配0个或任意多个字符（换行符除外））
x+        匹配至少一个x
x{n}      匹配确定的n个x（n是一个非负整数）
x{n,}     匹配至少n个x   #贪婪匹配（尽可能多的匹配）
x｛n，m｝ 匹配至少n个zuiduom个x，注意：n<=m
x|y       | 表示或，匹配的是x或y
"""


print(re.findall(r"(sunck)","sunck is a good man,sunck is a nice man"))
#print(re.findall(r"o?","sunck is a good man,sunck is a nice man"))#非贪婪匹配（尽可能少的匹配）
#print(re.findall(r"a*","aaabaa"))#贪婪匹配（尽可能多的匹配）
#print(re.findall(r"a+","aaaba"))
...
str = "sunck is a good man ,sunck is a nice man,sunck is a very good man"
...
print(re.findall(r"//*.*?/*/",r"/*  part1    */   /*  aprt2   */"))
